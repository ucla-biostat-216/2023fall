...
- In previous lecture, we study when does a linear equation $\mathbf{A} \mathbf{x} = \mathbf{b}$ have solution and how to characterize all the solutions. What if $\mathbf{A} \mathbf{x} = \mathbf{b}$ is **inconsistent**, that is there is no solution? 
...
$$
\nabla Q(\mathbf{b}) = -2 \mathbf{A}' \mathbf{b} + 2 \mathbf{A}' \mathbf{A} \mathbf{x} = \mathbf{0}.
$$
...
- The direct argument also reveals that the _fitted values_ $\mathbf{A} \hat{\mathbf{x}}$ is invariant to the choice of the solution to the normal equation.
...
- $\mathbf{A} (\mathbf{A}' \mathbf{A})^- \mathbf{A}'$ is the orthogonal projector onto $\mathcal{C}(\mathbf{A})$.
...
- Whichever least squares solution $\hat{\mathbf{x}}$ we use, the fitted value $\hat{\mathbf{b}} = \mathbf{A} \hat{\mathbf{x}} = \mathbf{A} (\mathbf{A}' \mathbf{A})^- \mathbf{A}' \mathbf{b}$ is the same.
...
- Step 1: Compute the (thin) QR factorization $\mathbf{A} = \mathbf{Q} \mathbf{R}$.
