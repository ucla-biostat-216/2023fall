...
Examples of inner product. **In class exercises**: express the following quantities using vector inner products.
...
- Assume vectors are all of length $n$. 
    ...
    1. Sum the elements of a vector: $n-1$ flops.
    
    2. Vector addition and substraction: $n$ flops.

    3. Scalar multiplication: $n$ flops.

    4. Elementwise multiplication: $n$ flops.

    5. Inner product: $2n - 1$ flops.  
    
    6. Norm of a vector: $2n$ flops.
...
Assume that the Apple M2 Max CPUs can do 8 double-precision flops per CPU cylce (?). Then the theoretical throughput of a single performance core on my laptop is
$$
8 \times 10^9 \times 16 = 27.9 \times 10^9 \text{ flops/second} = 27.9 \text{ GFLOPS}
$$
in double precision. I estimate my computer takes about
$$
\frac{10^7 - 1}{27.9 \times 10^9} \approx 0.000358 \text{ seconds} = 358 \text{ micro seconds}
$$
to sum a vector of length $n = 10^7$.
...
- Properties of L2 norm.
    ...
    There are at least 5 other proofs on [Wikipedia](https://en.wikipedia.org/wiki/Cauchy%E2%80%93Schwarz_inequality).
...
```{julia}
# check Cauchy-Schwartz inequality on random vectors
@show x = randn(5)
@show y = randn(5)
@show abs(x'y)
@show norm(x) * norm(y)
