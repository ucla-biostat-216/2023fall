...
- Now for the inhomogenous case $\mathbf{A} \mathbf{x} = \mathbf{b}$.
    ...
    &\Leftrightarrow& \mathbf{x} - \mathbf{A}^- \mathbf{b} = (\mathbf{I}_n - \mathbf{A}^- \mathbf{A}) \mathbf{q} \\
    &\Leftrightarrow& \mathbf{x} = \mathbf{A}^- \mathbf{b} + (\mathbf{I}_n - \mathbf{A}^- \mathbf{A}) \mathbf{q}.
    \end{eqnarray*}

- Corollary: $\mathbf{A} \mathbf{x} = \mathbf{b}$ is consistent for _all_ $\mathbf{b}$ if and only if $\mathbf{A}$ has full row rank.

- Corollary: If a system is consistent, its solution is unique if and only if $\mathbf{b}$ has full column rank.

## Invertible matrix

- If a square $\mathbf{A}$ has full row and column rank, then $\mathbf{A}$ is invertible (or nonsingular).

    The generalized inverse $\mathbf{A}^-$ is unique, denoted by $\mathbf{A}^{-1}$.
